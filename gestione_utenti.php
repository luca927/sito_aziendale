      ...
      <table class="table mb-0" id="tabellaUtenti">
        ...
        <tbody>
          ...
          <tr data-ruolo="<?= $u['ruolo'] ?>" data-search="<?= strtolower($u['username'].' '.$u['nome'].' '.$u['email']) ?>">
            ...
            <td>
              <?php
                $bc = match($u['ruolo']) { 'admin' => 'bg-danger', 'manager' => 'bg-warning text-dark', default => 'bg-primary' };
              ?>
      ...
      <div class="modal-body p-4">
        ...
        <div class="mb-2">
          ...
          <div class="input-group">
            ...
            <button class="btn btn-outline-secondary" type="button" onclick="generaPwd()"><i class="fa-solid fa-wand-magic-sparkles"></i></button>
          </div>
        </div>
      ...
      <div class="modal-body p-4">
        ...
        <div class="mb-2">
          ...
          <div class="input-group">
            ...
            <button class="btn btn-outline-secondary" type="button" onclick="generaPwdReset()"><i class="fa-solid fa-wand-magic-sparkles"></i></button>
          </div>
        </div>
        <div class="progress mb-1" style="height:5px;border-radius:10px;">
...
function showAlert(msg, type = 'success') {
    ...
    box.innerHTML = `<i class="fa-solid fa-${type === 'success' ? 'check-circle' : type === 'warning' ? 'triangle-exclamation' : 'circle-xmark'} me-2"></i>${msg}`;
...
function generaPwd() {
    ...
    const inp = document.getElementById('c_password');
    inp.type = 'text';
    inp.value = pwd;
    strengthCheck('c_password','c_bar','c_lbl');
}

function generaPwdReset() {
    const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789!@#$%';
    let pwd = '';
    for (let i = 0; i < 12; i++) pwd += chars[Math.floor(Math.random() * chars.length)];
    const inp = document.getElementById('r_password');
    inp.type = 'text';
    inp.value = pwd;
    strengthCheck('r_password','r_bar','r_lbl');
}
...
async function creaUtente() {
    ...
    try {
        const res = await fetch('api/get_utenti.php', {
            method: 'POST',
        ...
        if (data.success) {
            bootstrap.Modal.getInstance(document.getElementById('modalCrea')).hide();
...
async function resetPassword() {
    ...
    try {
        const res = await fetch('api/get_utenti.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'reset_pwd', id: document.getElementById('r_id').value, password: pwd })
        });
        const data = await res.json();
        if (data.success) {
...
async function eliminaUtente() {
    try {
        const res = await fetch('api/get_utenti.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'elimina', id: document.getElementById('e_id').value })
        });
        const data = await res.json();
        if (data.success) {
